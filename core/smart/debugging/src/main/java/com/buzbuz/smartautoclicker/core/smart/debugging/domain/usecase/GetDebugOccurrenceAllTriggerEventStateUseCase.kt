...
/** Get the state of all trigger events at the given event occurrence. */
@OptIn(ExperimentalCoroutinesApi::class)
class GetDebugOccurrenceAllTriggerEventStateUseCase @Inject constructor(
    private val smartRepository: IRepository,
    debuggingRepository: DebuggingRepository,
) {

    private val events: Flow<List<TriggerEvent>?> = debuggingRepository.getLastReportOverview()
        .map { overview ->
            overview ?: return@map null
            smartRepository.getTriggerEvents(overview.scenarioId)
        }

    private val eventOccurrences: Flow<List<DebugReportEventOccurrence>?> =
        debuggingRepository.getLastReportEventsOccurrences()

    operator fun invoke(eventOccurrence: DebugReportEventOccurrence): Flow<List<DebugEventOccurrenceEventState.Trigger>> =
