...
internal class TutorialsProviderImpl @Inject constructor() : TutorialsProvider {

    override val tutorialsInfo: Flow<List<TutorialInfo>>
        get() = flowOf(TutorialType.entries.map { tutorialType -> tutorialType.getTutorialInfo() })

    override fun getTutorial(id: String): Tutorial? =
        try { TutorialType.valueOf(id).getTutorial() }
