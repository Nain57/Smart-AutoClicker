...
class IsTriggerEventCopyAvailableUseCase @Inject constructor(
    @Dispatcher(IO) dispatcherIo: CoroutineDispatcher,
    editionRepository: EditionRepository,
    smartRepository: IRepository,
) {

    private val haveEditedEvents: Flow<Boolean> = editionRepository.editionState.editedTriggerEventsState
        .map { editedEventsState -> editedEventsState.value?.isNotEmpty() ?: false }

    private val haveDatabaseEvents: Flow<Boolean> = smartRepository.triggerEventsCount
        .flowOn(dispatcherIo)
