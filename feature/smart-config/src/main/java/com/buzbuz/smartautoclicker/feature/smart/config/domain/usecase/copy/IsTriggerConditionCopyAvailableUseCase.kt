...
class IsTriggerConditionCopyAvailableUseCase @Inject constructor(
    ...
    private val haveEditedConditions: Flow<Boolean> = editionRepository.editionState.editedTriggerEventsState
        .map { editedEvents -> editedEvents.value?.find { event -> event.conditions.isNotEmpty() } != null }

    private val haveDatabaseConditions: Flow<Boolean> = smartRepository.triggerConditionsCount
        .flowOn(dispatcherIo)
