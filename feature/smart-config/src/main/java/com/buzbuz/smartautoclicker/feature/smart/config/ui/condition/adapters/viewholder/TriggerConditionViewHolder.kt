...
class TriggerConditionViewHolder(
    parent: ViewGroup,
    val itemClickedListener: (item: UiTriggerCondition, index: Int) -> Unit,
): ViewBindingHolder<ItemTriggerConditionBinding>(
    viewBinding = ItemTriggerConditionBinding.inflate(LayoutInflater.from(parent.context), parent, false),
) {

    fun onBind(uiCondition: UiTriggerCondition) {
        viewBinding.apply {
            root.setOnClickListener { itemClickedListener(uiCondition, bindingAdapterPosition) }
