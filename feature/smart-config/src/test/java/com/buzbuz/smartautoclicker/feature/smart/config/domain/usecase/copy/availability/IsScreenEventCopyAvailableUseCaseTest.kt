...
class IsScreenEventCopyAvailableUseCaseTest {
    ...
    private val screenEventsCountFlow = MutableStateFlow(0)
    ...
    private lateinit var useCase: IsScreenEventCopyAvailableUseCase

    @OptIn(ExperimentalCoroutinesApi::class)
    @Before
    fun setUp() {
        every { mockEditionState.editedScreenEventsState } returns editedScreenEventsStateFlow
        every { mockSmartRepository.screenEventsCount } returns screenEventsCountFlow
        useCase = IsScreenEventCopyAvailableUseCase(
    ...
    @Test
    fun `edited events exist returns true`() = runTest {
    ...
    @Test
    fun `database events exist returns true`() = runTest {
        mockEditedEvents(emptyList())
        mockEventsCount(3)
    ...
    @Test
    fun `both edited and database events exist returns true`() = runTest {
        mockEditedEvents(listOf(mockk(relaxed = true)))
        mockEventsCount(2)
    ...
    private fun mockEventsCount(count: Int) {
        screenEventsCountFlow.value = count
    }
