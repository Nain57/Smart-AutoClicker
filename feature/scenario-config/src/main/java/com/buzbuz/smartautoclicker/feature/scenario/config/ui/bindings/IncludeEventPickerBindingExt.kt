...
fun IncludeEventPickerBinding.updateState(
    ...
    onPickerClicked: (List<ImageEvent>) -> Unit,
) {
    when (viewState) {
...
sealed class EventPickerViewState {
    ...
    data class NoSelection(val availableEvents: List<ImageEvent>): EventPickerViewState()
    data class Selected(val event: ImageEvent, val availableEvents: List<ImageEvent>): EventPickerViewState()
}